...
func NewRuleEngineServiceImpl(db *gorm.DB) *RuleEngineServiceImpl {
	...
	if common.GlobalMetrics != nil {
		common.GlobalMetrics.RuleEngineDuration.WithLabelValues("startup").Observe(0)
	}
...
func (s *RuleEngineServiceImpl) Scan(ctx context.Context, req *safeflow.ScanRequest) (resp *safeflow.ScanResponse, err error) {
	...
	if useAC && ac != nil {
		if result := ac.SearchWithRegex(req.Content); result != nil {
			...
			// 记录指标
			if common.GlobalMetrics != nil {
				common.GlobalMetrics.RuleMatchesTotal.WithLabelValues(result.Group, result.Action).Inc()
			}
			return resp, nil
		}
	} else {
	...
	if common.GlobalMetrics != nil {
		common.GlobalMetrics.RuleEngineDuration.WithLabelValues("ac_automaton").Observe(duration)
	}
